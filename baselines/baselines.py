...
class Pop:
    ...
    def __init__(self, top_n=25, item_key='reference', support_by_key=None):
        self.top_n = top_n
...
class ItemKNN:
    ...
    --------
    lmbd : float
        Regularization. Discounts the similarity of rare items (incidental co-occurrences). Default: 20
    alpha : float
        Balance between normalizing with the supports of the two items. 0.5 gives cosine similarity, 1.0 gives confidence (as in AR).
    '''

    def __init__(self, n_sims=250, lmbd=20, alpha=0.5, session_key='session_id', item_key='reference',
                 time_key='timestamp'):
        self.n_sims = n_sims
        self.lmbd = lmbd
        self.alpha = alpha
    ...
    def fit(self, data):
        data.set_index(np.arange(len(data)), inplace=True)  # reset index
        ...
        # support session
        supp = data.groupby('SessionIdx').size()  # session id: count
        session_offsets = np.zeros(len(supp) + 1, dtype=np.int32)
        session_offsets[1:] = supp.cumsum()  # start from 0
        ...
        for i in tqdm(range(n_items)):  # 8
            iarray = np.zeros(n_items)
            start = item_offsets[i]
            end = item_offsets[i + 1]

            # for every item, inside start:end
            for e in index_by_items[start:end]:
                uidx = data['SessionIdx'].values[e]  # session has this item
            ...
            iarray[i] = 0  # set item itself to 0
            norm = np.power((supp[i] + self.lmbd), self.alpha) * np.power((supp.values + self.lmbd), (
                    1. - self.alpha))  # (supp_i+λ)^α + (supp_j+λ)^(1-α)
            norm[norm == 0] = 1
            iarray = iarray / norm  # normalise
            indices = np.argsort(iarray)[-1:-1 - self.n_sims:-1]
            self.sims[itemids[i]] = pd.Series(data=iarray[indices], index=itemids[indices])

    def predict_next(self, session_id, input_item_id, predict_for_item_ids):
        preds = np.zeros(len(predict_for_item_ids))  # unique ids in train set
...
class BPR:
    ...
    def __init__(self, n_factors=100, n_iterations=10, learning_rate=0.01, lambda_session=0.0, lambda_item=0.0,
                 sigma=0.05, init_normal=False, session_key='session_id', item_key='reference'):
