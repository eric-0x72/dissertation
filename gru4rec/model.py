...
class GRU4Rec:
    ...
    def build_model(self):
        ...
        with tf.variable_scope('gru_layer'):
            ...
            embedding = tf.get_variable(name='embedding', shape=[self.n_items, self.rnn_size], initializer=initializer)
            softmax_W = tf.get_variable(name='softmax_w', shape=[self.n_items, self.rnn_size], initializer=initializer)
            softmax_b = tf.get_variable(name='softmax_b', shape=[self.n_items],
            ...
            cell_atten = tf.contrib.rnn.AttentionCellWrapper(drop_cell, attn_length=128, state_is_tuple=True)
            ...
            stacked_cell = rnn_cell.MultiRNNCell([drop_cell] * self.layers, state_is_tuple=True)  # __init__
            inputs = tf.nn.embedding_lookup(embedding, self.X)  # params, ids

            # print('self.state = ', self.state[0])
            output, state = stacked_cell(inputs=inputs, state=tuple(self.state))  # state=tuple(self.state)[0]
        ...
        if self.is_training:
            ...
            '''
            sampled_W = tf.nn.embedding_lookup(softmax_W, self.Y)  # params, ids
    ...
    def fit(self, data):
        ...
        print('fitting model...')
        print('no. of epoches = ', self.n_epochs)

        avgc_list = []  # list for epoch losses
            ...
            while not finished:
                ...
                for idx in mask:
                    ...
                    if maxiter >= len(offset_sessions) - 1:
                        # print('break condition met: ', maxiter, len(offset_sessions) - 1)
                        # print('Epoch {}\tStep {}\tlr: {:.6f}\tloss: {:.6f}'.format(epoch, step, lr, avgc))
                        avgc_list.append(avgc)
                        finished = True
                        break

                    # turn to next session data
                    iters[idx] = maxiter
    ...
    def predict_next_batch(self, session_ids, input_item_ids, itemidmap, batch=50):
        ...
        If the session ID at a given coordinate of the session_ids parameter remains the same during subsequent calls
            of the function, the corresponding hidden state of the network will be kept intact
        ...
        input_item_ids : 1D array
            Contains the item IDs of the events of the batch. Every item ID must be must be in the training data of
            the network. Its length must equal to the prediction batch size (batch param).
        batch : int
        ...
        out : pandas.DataFrame
            ...
            Columns: events of the batch
            rows: items. Rows are indexed by the item IDs.
        '''
