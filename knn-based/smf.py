...
class SessionMF:
    '''
    SessionMatrixFactorization( factors=100, batch=50, learn='adagrad_sub', learning_rate=0.001, momentum=0.0,
    regularization=0.0001, dropout=0.0, skip=0.0, samples=2048, activation='linear', objective='bpr_max_org',
    epochs=10, shuffle=3, last_n_days=None, session_key = 'SessionId', item_key= 'ItemId', time_key= 'Time' )
    ...
    def __init__(self, factors=10, batch=0, learn='adagrad_sub', learning_rate=0.001, momentum=0.0,
                 regularization=0.5, dropout=0.0, skip=0, samples=10, activation='linear', objective='bpr_max',
                 epochs=5, shuffle=-1, last_n_days=None, session_key='session_id', item_key='reference',
                 time_key='timestamp'):
    ...
    def fit(self, data, items=None):
        ...
        for j in range(self.epochs):
            ...
            while not finished:
                ...
                for i in range(self.batch):  # change batch to 0 works!!!
                    # print(sidx[i], spos[i])
                    # print(spos[i])
                    print('spops', spos[i])
                    print('shite ', self.session_map[self.sessions[sidx[i]]])
                    ...
                    if self.dropout > 0:
                        itemsl = list(items)
                        smat[i][itemsl] = smat[i][itemsl] * np.random.choice(2, size=len(itemsl),
                ...
                # if count % 10000 == 0 :
                #    print( 'finished {} of {} in epoch {} with loss {} / hr {} in {}s'
                #    .format( count, len(train), j, ( loss / count ), ( hit / count ), ( time.time() - start ) ) )

            print('finished epoch {} with loss {} / hr {} in {}s'.format(j, (loss / count), (hit / count),
                                                                         (time.time() - start)))
    ...
    def init_model(self, train, std=0.01):
        ...
        self.I = theano.shared(np.random.normal(0, std, size=(self.num_items, self.factors)).astype(self.floatX),
                               name='I')
        self.S = theano.shared(np.random.normal(0, std, size=(self.num_items, self.factors)).astype(self.floatX),
                               name='S')

        self.I1 = theano.shared(np.random.normal(0, std, size=(self.num_items, self.factors)).astype(self.floatX),
                                name='I1')
        self.I2 = theano.shared(np.random.normal(0, std, size=(self.num_items, self.factors)).astype(self.floatX),
                                name='I2')
    ...
    def _generate_train_model_batch_function(self):
        ...
        updates = getattr(self, self.learn)(cost, [self.S, self.I, self.I1, self.I2, self.BI, self.BS],
                                            [Sit, Iy, I1i, I2y, BIy, BSy], [items, y, i, y, y, y], self.learning_rate,
    ...
    def bpr_max_old(self, pred_mat, y):
        ...
        return T.cast(T.mean(-T.log(
            T.sum(T.nnet.sigmoid(T.diag(pred_mat.T) - pred_mat) * softmax_scores, axis=0) + 1e-24) + loss * T.sum(
            (pred_mat ** 2) * softmax_scores, axis=0)), self.floatX)

    def bpr_max(self, pred_mat, y):
        softmax_scores = self.softmax_neg(pred_mat).T
        return T.cast(T.mean(-T.log(T.sum(T.nnet.sigmoid(T.diag(pred_mat) - pred_mat.T) * softmax_scores,
                                          axis=0) + 1e-24) + self.regularization * T.sum(
            (pred_mat.T ** 2) * softmax_scores, axis=0)), self.floatX)
